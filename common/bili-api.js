...
class BiliAPI {
  ...
  encWbi(params, img_key, sub_key) {
    ...
    const queryString = Object.keys(queryParams)
      ...
      .map(
        (key) =>
          `${encodeURIComponent(key)}=${encodeURIComponent(queryParams[key].toString().replace(chr_filter, ""))}`,
      )
      .join("&");
  ...
  async getWbiKeys() {
    ...
    if (
      this.wbiKeys &&
      Date.now() - this.wbiKeysFetchTime < 12 * 60 * 60 * 1000
    ) {
      return this.wbiKeys;
    }
  ...
  async fetchVideoInfo(bvid) {
    ...
    const res = await fetch(
      `https://api.bilibili.com/x/web-interface/view/detail?${query}`,
      { credentials: "include" },
    );
  ...
  async fetchUserProfile(mid) {
    ...
    const [infoRes, statRes, upstatRes, relRes] = await Promise.allSettled([
      fetch(`https://api.bilibili.com/x/space/wbi/acc/info?${infoQuery}`, {
        credentials: "include",
      }).then((r) => r.json()),
      fetch(`https://api.bilibili.com/x/relation/stat?vmid=${mid}`, {
        credentials: "include",
      }).then((r) => r.json()),
      fetch(`https://api.bilibili.com/x/space/upstat?mid=${mid}`, {
        credentials: "include",
      }).then((r) => r.json()),
      fetch(`https://api.bilibili.com/x/space/wbi/acc/relation?${relQuery}`, {
        credentials: "include",
      }).then((r) => r.json()),
    ]);
    ...
    return {
      info:
        infoRes.status === "fulfilled" && infoRes.value.code === 0
          ? infoRes.value.data
          : null,
      stat:
        statRes.status === "fulfilled" && statRes.value.code === 0
          ? statRes.value.data
          : null,
      upstat:
        upstatRes.status === "fulfilled" && upstatRes.value.code === 0
          ? upstatRes.value.data
          : null,
      relation:
        relRes.status === "fulfilled" && relRes.value.code === 0
          ? relRes.value.data
          : null,
    };
  }
