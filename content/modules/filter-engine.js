...
class FilterEngine {
    ...
    return keywords.map((rule) => {
      try {
        ...
        if (rule.startsWith("/") && rule.lastIndexOf("/") > 0) {
          ...
          return new RegExp(
            rule.substring(1, lastSlash),
            rule.substring(lastSlash + 1),
          );
        }
        // 2. 解析通配符格式 (例: *关键词*)
        if (rule.includes("*")) {
  ...
  saveLog(title, up, reason, type, videoUrl, upUrl) {
    ...
    this.logTimer = setTimeout(() => {
      ...
      chrome.storage.local.get({ blockedLogs: [] }, ({ blockedLogs }) => {
        ...
        const newLogs = logsToSave.filter(
          (nl) =>
            !blockedLogs.some(
              (l) => l.title === nl.title && Date.now() - l.time < 5000,
            ),
        );
        // 仅保留最近 100 条记录
        chrome.storage.local.set({
          blockedLogs: [...newLogs, ...blockedLogs].slice(0, 100),
        });
      });
    }, 500);
  }
  ...
  removeCard(card, title, up, reason, type, videoUrl, upUrl) {
    const outer =
      card.closest(
        ".bili-feed-card, .feed-card, .video-card, .video-page-card-small",
      ) || card;
  ...
  async processCard(card) {
    // 避免重复处理或处理已被隐藏的卡片
    if (card.dataset.newbProcessed === "true" || card.style.display === "none")
      return;
    card.dataset.newbProcessed = "true";

    const titleEl = card.querySelector(
      ".bili-video-card__info--tit, .video-name, .title",
    );
    ...
    const upEl = card.querySelector(
      ".bili-video-card__info--author, .up-name__text, .up-name, .upname .name, .bili-video-card__info--owner, .bili-video-card__info--rcmd, .bili-live-card__info--author, .bili-live-card__info--uname-text",
    );
    let upLinkEl = card.querySelector('a[href*="space.bilibili.com"]');
    if (!upLinkEl) {
      upLinkEl = card.querySelector(
        'a.bili-video-card__info--owner, a.up-name, a[href*="live.bilibili.com"], a[href*="bangumi/play/"]',
      );
    }
    ...
    const upUrl =
      upLinkEl?.href && !upLinkEl.href.startsWith("javascript:")
        ? upLinkEl.href
        : "";
  ...
  async checkAdvanced(card, bvid, title, up, videoUrl, upUrl) {
    ...
    if (this.config.sectionRules.length > 0) {
      ...
      if (hitKw)
        return this.removeCard(card, title, up, tname, "分区", videoUrl, upUrl);
    }
