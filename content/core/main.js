(function () {
  ...
  const executeEngines = (config) => {
    ...
    const cards = document.querySelectorAll(
      ".feed-card, .bili-video-card, .video-card, .video-page-card-small",
    );
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (shouldRunFilter) {
      ...
      document
        .querySelectorAll('[data-newb-processed="true"]')
        .forEach((el) => {
  ...
  window.newbConfigManager
    .loadConfig()
    .then((cfg) => window.newbUIEngine?.update(cfg));
