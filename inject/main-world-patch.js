(function () {
  const isMasterOn = () =>
    localStorage.getItem("newb_master_switch") !== "false";
  const isLayoutOptEnabled = () =>
    isMasterOn() && localStorage.getItem("newb_layout_opt") === "true";
  const isShowIpEnabled = () =>
    isMasterOn() && localStorage.getItem("newb_show_ip") === "true";

  console.log("newB: Main World Patch Initializing...");

  /* ==========================================
   * Fetch & XHR 网络拦截器 (净化与防遥测)
   * ========================================== */
  const BLOCKED_URLS = [
  ...
  window.fetch = async function (input, init) {
    ...
    if (urlStr.includes(FEED_API)) {
      try {
        ...
        if (json.data?.item) {
          json.data.item = json.data.item.filter(
            (item) =>
              !["ad", "live"].includes(item.goto) &&
              item.card_type !== "ad_web_s",
          );
  ...
  XMLHttpRequest.prototype.send = function (...args) {
    if (
      isMasterOn() &&
      typeof this._newbUrl === "string" &&
      BLOCKED_URLS.some((blocked) => this._newbUrl.includes(blocked))
    ) {
      this.abort();
      return;
    }
  ...
  customElements.define = function (name, constructor, options) {
    ...
    const needsIpPatch =
      isShowIpEnabled() && name === "bili-comment-action-buttons-renderer";
        ...
        connectedCallback() {
          ...
          if (needsIpPatch) {
            ...
            if (location) {
              ...
              dynamicIpSheet.replaceSync(
                `#reply::after { content: "${location}"; }`,
              );
